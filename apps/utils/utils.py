...
def make_unique_slug(instance, title: str, slug_field: str = "slug"):
    ...
    queryset = Klass.objects.filter(**{f"{slug_field}__startswith": slug}).exclude(
        pk=instance.pk
    )
    similar_slugs = queryset.values_list(
        slug_field, flat=True
    )  # Slug maydonlar ro'yxatini oling
